...
export default function AdminCompraDetalle() {
  ...
  const itemsTotal = items.reduce((acc, it) => acc + (Number(it.precioUnitario || 0) * Number(it.cantidad ?? it.qty ?? 0)), 0)
  const neto = itemsTotal
        ...
        <div className="col-lg-5">
          <div className="card">
            <div className="card-body">
              ...
              <hr />
              <div className="d-flex justify-content-between"><strong>Total</strong><strong>{fmt(total)}</strong></div>
            </div>
          </div>
        </div>
                ...
                <table className="table align-middle">
                  ...
                  <tbody>
                    {items.map(it => {
                      ...
                      const subtotal = Number(it.precioUnitario || 0) * qty
                      return (
                        <tr key={it.id}>
                          <td>{it.nombre}</td>
