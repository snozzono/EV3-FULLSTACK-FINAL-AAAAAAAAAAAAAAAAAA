...
import AdminBlogForm from '../AdminBlogForm.jsx'
...
describe('AdminBlogs', () => {
  beforeEach(() => {
    ...
    global.fetch = vi.fn(async (url, opts) => {
      ...
      if (String(url).includes('/api/blogs/1') && opts?.method === 'DELETE') {
        const idx = blogs.findIndex(b => b.id === 1)
  ...
  it('lista blogs, crea y elimina', async () => {
      ...
      <AuthProvider>
        <BrowserRouter>
          <Routes>
            ...
            <Route path="/admin/blogs/nuevo" element={<AdminBlogForm />} />
          </Routes>
        </BrowserRouter>
      </AuthProvider>
    )
    ...
    await user.click(screen.getByRole('link', { name: 'Nuevo' }))
